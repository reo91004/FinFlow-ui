...
-   **XAI 분석**: 도메인 지식 기반의 휴리스틱 방식으로 포트폴리오 결정 요인 설명
    -   백테스팅 분석: 히스토리컬 데이터를 기반으로 안정적인 인사이트 제공
    -   라이브 분석 (준비 중): 실거래 환경 연동을 위한 차세대 분석 모드
...
1. **백테스팅 분석 (활성)**
    ...
    - 히스토리컬 데이터와 IRT 입력값을 사용하여 안정적인 인사이트를 제공
    - 사용자 온보딩 단계에서 선택되며 현재 기본 모드로 동작
    - 성과 지표, 상관관계, 위험도 등을 한 번에 계산하여 설명 생성

2. **라이브 분석 (준비 중)**
    - 실거래 계좌 및 실시간 시세 연동을 목표로 하는 차세대 모드
    - 현재 UI에서 버튼만 노출되고 실제 분석은 백테스팅 모드로 대체 실행
    - 추후 실시간 리밸런싱 시나리오에 맞춰 활성화 예정
